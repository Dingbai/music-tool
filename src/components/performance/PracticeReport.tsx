...
import { Card, Row, Col, Statistic, Progress, Button, Divider, Space } from 'antd';
import {
  TrophyOutlined,
  FireOutlined,
  CheckCircleOutlined,
...
const PracticeReport: React.FC<PracticeReportProps> = ({
  ...
  const total = hitNotes + missNotes;
  ...
  return (
    ...
    >
      ...
      <div style={{ marginBottom: 24 }}>
        <div style={{ fontSize: 14, color: '#666', marginBottom: 8 }}>总体表现</div>
        <Progress
          percent={accuracy}
          strokeColor={{
            '0%': accuracy >= 80 ? '#52c41a' : accuracy >= 60 ? '#faad14' : '#ff4d4f',
            '100%': accuracy >= 80 ? '#73d13d' : accuracy >= 60 ? '#ffc53d' : '#ff7875',
          }}
      ...
      <Space direction="vertical" style={{ width: '100%' }} size="middle">
        <Button type="primary" size="large" block icon={<RedoOutlined />} onClick={onPlayAgain}>
          再来一次
        </Button>
      </Space>
    </Card>
